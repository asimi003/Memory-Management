...
Person::Person(const char *name_, Person* father_, Person* mother_){
    this->name = new char[strlen(name_)+1]; //add 1  to ever new name
    //we add 1 because the every string needs to include a null character
    strcpy(name, name_);
    father = father_;
    mother = mother_;
    capacity = 1;
...
Person::~Person(){
	delete[] children; // added because children was a new[]
	delete[] name; //added 
}
...
void Person::printLineage(char dir, int level){
    ...
    delete[] temp; //need to add a delete[] because of char pointer temp array
}
...
void expand(Person ***t, int *MAX){
  ...
  delete[] *t;  //I added here since t's type is will be a pointer to an array of pointers
  *MAX *= 2;
  *t = temp;
}
